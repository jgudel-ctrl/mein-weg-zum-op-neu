...
test('enthält 17 neu erstellte, hochauflösende Illustrationen', () => {
...
test('stellt rund 40 europäische und gängige Sprachen mit je 17 Schritten bereit', async () => {
  ...
  for (const { code, nativeName, flag } of i18n.languages) {
    assert.ok(nativeName.length > 1, `${code}: Eigenbezeichnung fehlt`);
    assert.ok(flag, `${code}: Landesflagge fehlt`);
    assert.equal(i18n.translations[code].steps.length, 17, `${code}: keine 17 Schritte`);
...
test('bildet das klinische Inhaltsfeedback in Deutsch und der Reihenfolge ab', async () => {
  ...
  assert.deepEqual(i18n.stepOrder, [0, 1, 8, 2, 3, 4, 5, 15, 6, 7, 9, 10, 11, 12, 13, 14, 16]);
  ...
  assert.match(steps[2].description, /Gurt.*Transport/);
  ...
  assert.doesNotMatch(steps[13].description, /\(Braunüle\)/);
  assert.equal(steps[15].title, 'Nicht rauchen');
  assert.equal(steps[15].description, 'Bitte rauchen Sie vor der Operation nicht.');
  assert.equal(steps[16].title, 'Alles verstanden?');
  assert.equal(steps[16].description, 'Haben Sie noch Fragen oder ist etwas unklar? Bitte sprechen Sie das Stationspersonal an. Wir helfen Ihnen gerne weiter.');
